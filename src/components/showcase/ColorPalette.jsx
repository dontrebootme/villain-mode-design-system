...
const ColorSwatch = ({ name, hex }) => {
  ...
  return (
    ...
    >
      <div
        className="h-24 mb-3 border border-facet-grey hover:border-infinity-gold transition-colors duration-300"
        style={{ backgroundColor: hex }}
      ></div>
      <div className="text-sm">
        <p className="font-heading text-starlight-white mb-1">{name}</p>
        <p className="font-code text-xs text-faded-silver">{hex}</p>
        {copied && (
          <p className="text-xs text-infinity-gold mt-1">Copied!</p>
        )}
      </div>
    </div>
  );
...
const ColorCategory = ({ title, description, colorSet }) => {
  return (
    <div className="mb-12">
      <h3 className="text-2xl mb-2 text-ultron-cyan">{title}</h3>
      <p className="text-faded-silver mb-6">{description}</p>
